...
def loadCNN(bTraining=False):
    ...
    if not bTraining:
        ...
        w = int(input(
            "Which weight file to load (enter the INDEX of it, which starts from 0): "))
...
def visualizeLayer(model, img, input_image, layerIndex):
    ...
    if output_image.ndim == 4:
        ...
        print("Dumping filter data of layer{} - {}".format(layerIndex,
        ...
        # plt.show()
        savedfilename = "img_" + \
            str(img) + "_layer" + str(layerIndex) + \
            "_"+layer.__class__.__name__+".png"
        fig.savefig(savedfilename)
        print("Create file - {}".format(savedfilename))
        # plt.close(fig)
    else:
        print("Can't dump data of this layer{}- {}".format(layerIndex,
              layer.__class__.__name__))
